/*
    ...
    - LZ5 source mirror : https://github.com/inikep/lz5
...
#   define FUZ_DISPLAYTEST          { testNb++; g_displayLevel<3 ? 0 : printf("%2u\b\b", testNb); if (g_displayLevel==4) fflush(stdout); }
    void* stateLZ5   = malloc(LZ5_sizeofState());
    void* stateLZ5HC = malloc(LZ5_sizeofStateHC());
    LZ5_stream_t LZ5dict;
    LZ5_streamHC_t LZ5dictHC;
    ...
    /* init */
    memset(&LZ5dict, 0, sizeof(LZ5dict));
    duration *= 1000;
    ...
    for (cycleNb = 0; cycleNb < startCycle; cycleNb++)
    {
        ...
        if (0)   /* some problems are related to dictionary re-use; in this case, enable this loop */
        {
            ...
            LZ5_compress_continue(&LZ5dict, block, compressedBuffer, blockSize);
            LZ5_loadDict(&LZ5dict, dict, dictSize);
            LZ5_compress_continue(&LZ5dict, block, compressedBuffer, blockSize);
            LZ5_loadDict(&LZ5dict, dict, dictSize);
            LZ5_compress_continue(&LZ5dict, block, compressedBuffer, blockSize);
    ...
    for (cycleNb = startCycle; (cycleNb < nbCycles) || (FUZ_GetMilliSpan(startTime) < duration) ; cycleNb++)
    {
        ...
        FUZ_DISPLAYTEST;
        {
            ...
            ret = LZ5_compress_destSize(block, compressedBuffer, &srcSize, targetSize);
            FUZ_CHECKTEST(ret > targetSize, "LZ5_compress_destSize() result larger than dst buffer !");
            FUZ_CHECKTEST(compressedBuffer[targetSize] != endCheck, "LZ5_compress_destSize() overwrite dst buffer !");
            FUZ_CHECKTEST(srcSize > blockSize, "LZ5_compress_destSize() fed more than src buffer !");
            DISPLAYLEVEL(5, "destSize : %7i/%7i; content%7i/%7i ", ret, targetSize, srcSize, blockSize);
            if (targetSize>0)
            {
                FUZ_CHECKTEST((ret==0), "LZ5_compress_destSize() compression failed");
                /* check correctness */
                FUZ_DISPLAYTEST;
                ...
                ret = LZ5_decompress_safe(compressedBuffer, decodedBuffer, compressedSize, srcSize);
                FUZ_CHECKTEST(ret<0, "LZ5_decompress_safe() failed on data compressed by LZ5_compress_destSize");
                FUZ_CHECKTEST(ret!=srcSize, "LZ5_decompress_safe() failed : did not fully decompressed data");
                FUZ_CHECKTEST(decodedBuffer[srcSize] != endCheck, "LZ5_decompress_safe() overwrite dst buffer !");
        ...
        /* Test compression HC */
        FUZ_DISPLAYTEST;
        ret = LZ5_compressHC(block, compressedBuffer, blockSize);
        FUZ_CHECKTEST(ret==0, "LZ5_compressHC() failed");
        HCcompressedSize = ret;

        /* Test compression HC using external state */
        FUZ_DISPLAYTEST;
        ret = LZ5_compressHC_withStateHC(stateLZ5HC, block, compressedBuffer, blockSize);
        FUZ_CHECKTEST(ret==0, "LZ5_compressHC_withStateHC() failed");

        /* Test compression using external state */
        FUZ_DISPLAYTEST;
        ret = LZ5_compress_withState(stateLZ5, block, compressedBuffer, blockSize);
        FUZ_CHECKTEST(ret==0, "LZ5_compress_withState() failed");

        /* Test compression */
        FUZ_DISPLAYTEST;
        ret = LZ5_compress(block, compressedBuffer, blockSize);
        ...
        FUZ_DISPLAYTEST;
        ret = LZ5_compress_limitedOutput(block, compressedBuffer, blockSize, compressedSize);
        FUZ_CHECKTEST(ret==0, "LZ5_compress_limitedOutput() failed despite sufficient space");

        /* Test compression with output size being exactly what's necessary and external state (should work) */
        FUZ_DISPLAYTEST;
        ret = LZ5_compress_limitedOutput_withState(stateLZ5, block, compressedBuffer, blockSize, compressedSize);
        FUZ_CHECKTEST(ret==0, "LZ5_compress_limitedOutput_withState() failed despite sufficient space");

        /* Test HC compression with output size being exactly what's necessary (should work) */
        FUZ_DISPLAYTEST;
        ret = LZ5_compressHC_limitedOutput(block, compressedBuffer, blockSize, HCcompressedSize);
        FUZ_CHECKTEST(ret==0, "LZ5_compressHC_limitedOutput() failed despite sufficient space");

        /* Test HC compression with output size being exactly what's necessary (should work) */
        FUZ_DISPLAYTEST;
        ret = LZ5_compressHC_limitedOutput_withStateHC(stateLZ5HC, block, compressedBuffer, blockSize, HCcompressedSize);
        FUZ_CHECKTEST(ret==0, "LZ5_compressHC_limitedOutput_withStateHC() failed despite sufficient space");
        ...
        FUZ_DISPLAYTEST;
        {
            ...
            ret = LZ5_compress_limitedOutput(block, compressedBuffer, blockSize, compressedSize-missingBytes);
            FUZ_CHECKTEST(ret, "LZ5_compress_limitedOutput should have failed (output buffer too small by %i byte)", missingBytes);
            FUZ_CHECKTEST(compressedBuffer[compressedSize-missingBytes], "LZ5_compress_limitedOutput overran output buffer ! (%i missingBytes)", missingBytes)
        }
        ...
        FUZ_DISPLAYTEST;
        {
            ...
            ret = LZ5_compressHC_limitedOutput(block, compressedBuffer, blockSize, HCcompressedSize-missingBytes);
            FUZ_CHECKTEST(ret, "LZ5_compressHC_limitedOutput should have failed (output buffer too small by %i byte)", missingBytes);
            FUZ_CHECKTEST(compressedBuffer[HCcompressedSize-missingBytes], "LZ5_compressHC_limitedOutput overran output buffer ! (%i missingBytes)", missingBytes)
        }
        ...
        FUZ_DISPLAYTEST;
        {
            ...
            LZ5_compress_continue (&LZ5_stream, dict, compressedBuffer, dictSize);   /* Just to fill hash tables */
            blockContinueCompressedSize = LZ5_compress_continue (&LZ5_stream, block, compressedBuffer, blockSize);
        ...
        FUZ_CHECKTEST(ret!=blockContinueCompressedSize, "LZ5_decompress_fast_withPrefix64k did not read all compressed block input");
        ...
        FUZ_CHECKTEST(crcCheck!=crcOrig, "LZ5_decompress_fast_withPrefix64k corrupted decoded data (dict %i)", dictSize);
        ...
        FUZ_CHECKTEST(ret!=blockSize, "LZ5_decompress_safe_usingDict did not regenerate original data");
        crcCheck = XXH32(decodedBuffer+dictSize, blockSize, 0);
        ...
        LZ5_loadDict(&LZ5dict, dict, dictSize);
        blockContinueCompressedSize = LZ5_compress_continue(&LZ5dict, block, compressedBuffer, blockSize);
        FUZ_CHECKTEST(blockContinueCompressedSize==0, "LZ5_compress_continue failed");

        FUZ_DISPLAYTEST;
        LZ5_loadDict(&LZ5dict, dict, dictSize);
        ret = LZ5_compress_limitedOutput_continue(&LZ5dict, block, compressedBuffer, blockSize, blockContinueCompressedSize-1);
        FUZ_CHECKTEST(ret>0, "LZ5_compress_limitedOutput_continue using ExtDict should fail : one missing byte for output buffer : %i written, %i buffer", ret, blockContinueCompressedSize);

        FUZ_DISPLAYTEST;
        LZ5_loadDict(&LZ5dict, dict, dictSize);
        ret = LZ5_compress_limitedOutput_continue(&LZ5dict, block, compressedBuffer, blockSize, blockContinueCompressedSize);
        ...
        FUZ_CHECKTEST(ret<=0, "LZ5_compress_limitedOutput_continue should work : enough size available within output buffer");
        ...
        if (dict < (char*)CNBuffer) dict = (char*)CNBuffer;
        LZ5_resetStreamHC (&LZ5dictHC, FUZ_rand(&randState) & 0x7);
        LZ5_loadDictHC(&LZ5dictHC, dict, dictSize);
        blockContinueCompressedSize = LZ5_compressHC_continue(&LZ5dictHC, block, compressedBuffer, blockSize);
        FUZ_CHECKTEST(blockContinueCompressedSize==0, "LZ5_compressHC_continue failed");

        FUZ_DISPLAYTEST;
        LZ5_loadDictHC(&LZ5dictHC, dict, dictSize);
        ret = LZ5_compressHC_limitedOutput_continue(&LZ5dictHC, block, compressedBuffer, blockSize, blockContinueCompressedSize-1);
        FUZ_CHECKTEST(ret>0, "LZ5_compressHC_limitedOutput_continue using ExtDict should fail : one missing byte for output buffer");

        FUZ_DISPLAYTEST;
        LZ5_loadDictHC(&LZ5dictHC, dict, dictSize);
        ret = LZ5_compressHC_limitedOutput_continue(&LZ5dictHC, block, compressedBuffer, blockSize, blockContinueCompressedSize);
        FUZ_CHECKTEST(ret!=blockContinueCompressedSize, "LZ5_compress_limitedOutput_compressed size is different (%i != %i)", ret, blockContinueCompressedSize);
        FUZ_CHECKTEST(ret<=0, "LZ5_compress_limitedOutput_continue should work : enough size available within output buffer");
...
static void FUZ_unitTests(void)
{
    ...
    /* LZ5 streaming tests */
    {
        ...
        /* Allocation test */
        statePtr = LZ5_createStream();
        FUZ_CHECKTEST(statePtr==NULL, "LZ5_createStream() allocation failed");
        LZ5_freeStream(statePtr);

        /* simple compression test */
        crcOrig = XXH64(testInput, testCompressedSize, 0);
        LZ5_resetStream(&streamingState);
        result = LZ5_compress_limitedOutput_continue(&streamingState, testInput, testCompressed, testCompressedSize, testCompressedSize-1);
        FUZ_CHECKTEST(result==0, "LZ5_compress_limitedOutput_continue() compression failed");
        ...
        /* ring buffer test */
        {
            ...
            while (iNext + messageSize < testCompressedSize)
            {
                ...
                result = LZ5_compress_limitedOutput_continue(&streamingState, ringBuffer + rNext, testCompressed, messageSize, testCompressedSize-ringBufferSize);
                FUZ_CHECKTEST(result==0, "LZ5_compress_limitedOutput_continue() compression failed");
    ...
    /* LZ5 HC streaming tests */
    {
        ...
        /* Allocation test */
        sp = LZ5_createStreamHC();
        FUZ_CHECKTEST(sp==NULL, "LZ5_createStreamHC() allocation failed");
        LZ5_freeStreamHC(sp);

        /* simple HC compression test */
        crcOrig = XXH64(testInput, testCompressedSize, 0);
        LZ5_resetStreamHC(&sHC, 0);
        result = LZ5_compressHC_limitedOutput_continue(&sHC, testInput, testCompressed, testCompressedSize, testCompressedSize-1);
        FUZ_CHECKTEST(result==0, "LZ5_compressHC_limitedOutput_continue() compression failed");
        ...
        LZ5_resetStreamHC(&sHC, 0);
        LZ5_loadDictHC(&sHC, testInput, 64 KB);
        result = LZ5_compressHC_limitedOutput_continue(&sHC, testInput + 64 KB, testCompressed, testCompressedSize, testCompressedSize-1);
        FUZ_CHECKTEST(result==0, "LZ5_compressHC_limitedOutput_continue() dictionary compression failed : result = %i", result);

        result = LZ5_decompress_safe_usingDict(testCompressed, testVerify, result, testCompressedSize, testInput, 64 KB);
        ...
        /* multiple HC compression test with dictionary */
        {
            ...
            LZ5_resetStreamHC(&sHC, 0);
            LZ5_loadDictHC(&sHC, testInput, segSize);
            result1 = LZ5_compressHC_limitedOutput_continue(&sHC, testInput + segSize, testCompressed, segSize, segSize -1);
            FUZ_CHECKTEST(result1==0, "LZ5_compressHC_limitedOutput_continue() dictionary compression failed : result = %i", result1);
            result2 = LZ5_compressHC_limitedOutput_continue(&sHC, testInput + 2*segSize, testCompressed+result1, segSize, segSize-1);
            FUZ_CHECKTEST(result2==0, "LZ5_compressHC_limitedOutput_continue() dictionary compression failed : result = %i", result2);
        ...
        crcOrig = XXH64(testInput + 64 KB, testCompressedSize, 0);
        LZ5_resetStreamHC(&sHC, 0);
        LZ5_loadDictHC(&sHC, testInput, 32 KB);
        result = LZ5_compressHC_limitedOutput_continue(&sHC, testInput + 64 KB, testCompressed, testCompressedSize, testCompressedSize-1);
        FUZ_CHECKTEST(result==0, "LZ5_compressHC_limitedOutput_continue() remote dictionary failed : result = %i", result);
        ...
        /* multiple HC compression with ext. dictionary */
        {
            ...
            LZ5_resetStreamHC(&sHC, 0);
            LZ5_loadDictHC(&sHC, dict, dictSize);
            ...
            while (segStart + segSize < testInputSize)
            {
                ...
                result = LZ5_compressHC_limitedOutput_continue(&sHC, testInput + segStart, testCompressed, segSize, LZ5_compressBound(segSize));
                FUZ_CHECKTEST(result==0, "LZ5_compressHC_limitedOutput_continue() dictionary compression failed : result = %i", result);
        ...
        /* ring buffer test */
        {
            ...
            while (iNext + messageSize < testCompressedSize)
            {
                ...
                memcpy (ringBuffer + rNext, testInput + iNext, messageSize);
                result = LZ5_compressHC_limitedOutput_continue(&sHC, ringBuffer + rNext, testCompressed, messageSize, testCompressedSize-ringBufferSize);
                FUZ_CHECKTEST(result==0, "LZ5_compressHC_limitedOutput_continue() compression failed");
...
#define BSIZE2 16435
            ...
            /* first block */
                ...
                result = LZ5_compressHC_limitedOutput_continue(&sHC, testInput + iNext, testCompressed, messageSize, testCompressedSize-ringBufferSize);
                FUZ_CHECKTEST(result==0, "LZ5_compressHC_limitedOutput_continue() compression failed");
...
int main(int argc, char** argv)
{
    ...
    printf("Starting LZ5 fuzzer (%i-bits, %s)\n", (int)(sizeof(size_t)*8), LZ5_VERSION);
