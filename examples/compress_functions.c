 ...
 *
 *              The call stack (before theoretical compiler optimizations) for LZ5_compress_Level1 is as follows:
 *                LZ5_compress_Level1
 *                  LZ5_compress_fast
...
uint64_t bench(
  ...
  LZ5_stream_t* state = LZ5_createStream_Level1();
  if (!state) return;
  ...
  switch(function_id) {
    case ID__LZ5_COMPRESS_DEFAULT:
      printf("Starting benchmark for function: LZ5_compress_Level1()\n");
      for(int junk=0; junk<warm_up; junk++)
        rv = LZ5_compress_Level1(src, dst, src_size, max_dst_size);
      if (rv < 1)
        run_screaming("Couldn't run LZ5_compress_Level1()... error code received is in exit code.", rv);
...
//      LZ5_resetStream_Level1((LZ5_stream_t*)state);
//      for(int junk=0; junk<warm_up; junk++) {
//        LZ5_resetStream_Level1((LZ5_stream_t*)state);
...
int main(int argc, char **argv) {
  ...
  bytes_returned = LZ5_compress_Level1(src, known_good_dst, src_size, max_dst_size);
  if (bytes_returned < 1)
  ...
  bytes_returned = LZ5_compress_Level1(hc_src, known_good_hc_dst, src_size, max_dst_size);
  if (bytes_returned < 1)
  ...
  LZ5_stream_t* state = LZ5_createStream_Level1();
  if (!state) return;
  bytes_returned = LZ5_compress_extState_Level1(state, src, dst, src_size, max_dst_size, 1);
  if (bytes_returned < 1)
    run_screaming("Failed to compress src using LZ5_compress_extState_Level1.  echo $? for return code.", bytes_returned);
  if (memcmp(dst, known_good_dst, bytes_returned) != 0)
    run_screaming("According to memcmp(), the value we got in dst from LZ5_compress_extState_Level1 doesn't match the known-good value.  This is bad.", 1);
  ...
  */
  ...
  printf("%s", separator);
  printf(format, "Normal Text", "LZ5_compress_Level1()",       (double)time_taken__default       / BILLION, (int)(iterations / ((double)time_taken__default       /BILLION)), time_taken__default       / iterations, (double)time_taken__default       * 100 / time_taken__default);
